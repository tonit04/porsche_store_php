...
<main>
    <!-- 1. BANNER ĐỘNG (Carousel từ bảng banners) -->
    <section class="my-5">
                ...
                <div class="carousel-inner">
                    <?php foreach ($banners as $index => $banner): ?>
                        <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                            <div style="background-image: url('assets/images/banners/<?= htmlspecialchars($banner->image_url) ?>'); height: 60vh; background-size: cover; background-position: center;"
                                class="d-block w-100">
                                <div class="carousel-caption d-none d-md-block text-center"
                                    style="background-color: rgba(0,0,0, 0.3); padding: 20px; bottom: 10%;">
                                    <h2><?= htmlspecialchars($banner->name) ?></h2>
        ...
        <?php else: ?>
            <div class="container text-center my-5">
                <p>Hiện chưa có banner nào để hiển thị.</p>
            </div>
        <?php endif; ?>
    </section>
            ...
            <div id="carouselXeMoi" class="carousel slide" data-bs-ride="false">
                <div class="carousel-inner">
                    <div class="carousel-item active">
                        ...
                        ?>
                        <div style="background-image: url('<?php echo $fullLatestCarImagePath; ?>'); height: 75vh; background-size: cover; background-position: center; position: relative;"
                            class="d-block w-100">
                            <div class="container h-100 d-flex flex-column justify-content-end pb-5 text-white"
                                style="text-shadow: 1px 1px 3px rgba(0,0,0,0.7);">
                                <h1 class="display-4 fw-bold">
                                    <a href="<?php echo BASE_URL; ?>index.php?controller=product&action=Details&<?php echo !empty($latestCar->slug) ? 'slug=' . htmlspecialchars($latestCar->slug) : 'id=' . htmlspecialchars($latestCar->id); ?>"
                                ...
                                <?php if (isset($latestCar->model) && is_object($latestCar->model) && !empty($latestCar->model->name)): ?>
                                    <p class="lead mb-1"><small>Model:
                                            <?php echo htmlspecialchars($latestCar->model->name); ?></small></p>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($latestCar->description)): ?>
                                    <p class="mb-1">
                                        <small><?php echo nl2br(htmlspecialchars(substr($latestCar->description, 0, 150))); ?><?php echo strlen($latestCar->description) > 150 ? '...' : ''; ?></small>
                                    </p>
                                <?php endif; ?>
                                <div class="mt-4">
                                    <a href="<?php echo BASE_URL; ?>index.php?controller=product&action=Details&<?php echo !empty($latestCar->slug) ? 'slug=' . htmlspecialchars($latestCar->slug) : 'id=' . htmlspecialchars($latestCar->id); ?>"
                                        class="btn btn-danger btn-lg">Tìm hiểu thêm</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col">
                        <div class="card h-100 shadow-sm border-0 text-center">
                            ...
                            <a href="<?php echo BASE_URL; ?>index.php?controller=product&action=list&category_id=<?php echo htmlspecialchars($category->id); ?>"
                                ...
                                <img src="<?php echo $fullImagePath; ?>" class="card-img-top p-3"
                                    alt="<?php echo htmlspecialchars($category->name); ?>"
                                    style="aspect-ratio: 16/9; object-fit: contain; max-height: 200px;">
                            ...
                            <div class="card-footer bg-transparent border-0 pb-3">
                                <a href="<?php echo BASE_URL; ?>index.php?controller=product&action=list&category_id=<?php echo htmlspecialchars($category->id); ?>"
                                    class="btn btn-outline-danger btn-sm">Xem các mẫu
